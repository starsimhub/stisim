...
class Trichomoniasis(SEIS):
    ...
    def set_duration(self, p, symp, asymp, pid):
        ...
        # Convert to timesteps before adding: ti_infected is an index, so the duration has to be
        # expressed in timesteps, not years. The base class does this via set_pars(); this override
        # has to do it explicitly.
        dur_persist = self.pars.dur_persist.to_dt()
        self.ti_clearance[f_persist] = self.ti_infected[f_persist] + dur_persist
        return
