...
class HIVPars(BaseSTIPars):
    def __init__(self, **kwargs):
        ...
        self.dur_acute = ss.lognorm_ex(
            ss.months(3), ss.months(1)
        )  # Duration of acute HIV infection
        self.dur_latent = ss.lognorm_ex(
            ss.years(10), ss.years(3)
        )  # Duration of latent, untreated HIV infection
        self.dur_falling = ss.lognorm_ex(
            ss.years(3), ss.years(1)
        )  # Duration of late-stage HIV when CD4 counts fall
        self.p_hiv_death = ss.bernoulli(
            p=0
        )  # Death from HIV-related complications; set by make_p_hiv_death()
        ...
        self.rel_trans_acute = ss.normal(
            loc=6, scale=0.5
        )  # Increase transmissibility during acute HIV infection
        self.rel_trans_falling = ss.normal(
            loc=8, scale=0.5
        )  # Increase transmissibility during late HIV infection
        self.eff_condom = 0.9
        ...
        # Care seeking
        self.care_seeking = ss.normal(
            loc=1, scale=0.1
        )  # Distribution of relative care-seeking behavior
        self.maternal_care_scale = (
            2  # Factor for scaling up care-seeking behavior during pregnancy
        )
        ...
        self.time_to_art_efficacy = ss.months(
            6
        )  # Time to reach full ART efficacy (in months) - linear increase in efficacy
...
class HIV(BaseSTI):
    ...
    def __init__(self, pars=None, init_prev_data=None, **kwargs):
        ...
        self.define_states(
            ...
            ss.BoolState(
                "post_art"
            ),  # After stopping ART, CD4 falls linearly until death
            ss.FloatArr(
                "ti_zero"
            ),  # Time of zero CD4 count - generally corresponds to AIDS death
            ...
            ss.FloatArr(
                "cd4_start"
            ),  # Initial CD4 count for each agent before an infection
    ...
    @property
    def include_mtct(self):
        return "pregnancy" in self.sim.demographics
    ...
    def init_results(self):
        ...
        results = [
            ...
            ss.Result(
                "prevalence_15_49", dtype=float, label="Prevalence 15-49", scale=False
            ),
            ss.Result(
                "prevalence_sw", dtype=float, label="FSW prevalence", scale=False
            ),
            ss.Result("new_infections_sw", dtype=int, label="New infections - FSW"),
            ss.Result(
                "new_infections_not_sw", dtype=int, label="New infections - Other F"
            ),
            ss.Result(
                "prevalence_client", dtype=float, label="Client prevalence", scale=False
            ),
            ss.Result(
                "new_infections_client", dtype=int, label="New infections - Clients"
            ),
            ss.Result(
                "new_infections_not_client", dtype=int, label="New infections - Other M"
            ),
        ...
        if "structuredsexual" in self.sim.networks.keys():
            for risk_group in range(
                self.sim.networks.structuredsexual.pars.n_risk_groups
            ):
                for sex in ["female", "male"]:
                    results += [
                        ss.Result(
                            "prevalence_risk_group_" + str(risk_group) + "_" + sex,
                            scale=False,
                        ),
                        ss.Result(
                            "new_infections_risk_group_" + str(risk_group) + "_" + sex,
                            dtype=int,
                        ),
                    ]
    ...
    def init_post(self):
        ...
        ss.Module.init_post(
            self
        )  # Skip the disease init_post() since we create infections in a different way
        # Set initial CD4
        self.init_cd4()
        ...
        if self.init_prev_data is not None:
            ...
            self.pars.init_prev.set(
                p_init_infection
            )  # Set the initial prevalence function
    ...
    def acute_decline(self, uids):
        ...
        acute_dur = (
            acute_end - acute_start
        )  # Total time in acute phase, rounded to timesteps
    ...
    def post_art_decline(self, uids):
        ...
        cd4[zero_later_inds] = np.maximum(
            cd4_end, cd4_start - per_timestep_decline * time_post_art
        )
        return cd4
    ...
    def cd4_increase(self, uids):
        ...
        cd4_now = (
            2 * cd4_total_gain / (1 + np.exp(-dur_art * growth_rate))
            - cd4_total_gain
            + cd4_preart
        )  # Concave logistic
    ...
    def make_p_hiv_death(self, uids=None):
        ...
        p_hiv_death = ss.peryear(
            np.array([0.003, 0.003, 0.005, 0.01, 0.05, 0.300])
        ).to_prob(self.dt)
    ...
    def _interpolate(vals: list, t):
        ...
        assert len({x[0] for x in vals}) == len(
            vals
        )  # Make sure time points are unique
        return np.interp(
            t,
            [x[0] for x in vals],
            [x[1] for x in vals],
    ...
    def init_care_seeking(self):
        ...
        self.baseline_care_seeking[uids] = sc.dcp(
            self.care_seeking[uids]
        )  # Copy it so pregnancy can modify it
        return
    ...
    def step_state(self):
        ...
        if self.include_mtct:
            ...
            self.care_seeking[pregnant] = (
                self.baseline_care_seeking[pregnant] * self.pars.maternal_care_scale
            )
        ...
        if untreated_falling.any():
            self.cd4[untreated_falling.uids] = self.falling_decline(
                untreated_falling.uids
            )

        # Update CD4 nadir for anyone not on treatment
        untreated = self.infected & ~self.on_art
        self.cd4_nadir[untreated] = np.minimum(
            self.cd4_nadir[untreated], self.cd4[untreated]
        )
    ...
    def update_transmission(self):
        ...
        if self.on_art.any():
            ...
            efficacy_to_date[new_on_art] = (
                timesteps_on_art[new_on_art] * full_eff / time_to_full_eff.value
            )
    ...
    def update_results(self):
        ...
        self.results["cum_diagnoses"][ti] = np.sum(
            self.results["new_diagnoses"][: ti + 1]
        )
        self.results["new_agents_on_art"][ti] = np.count_nonzero(self.ti_art == ti)
        if self.include_mtct:
            self.results["n_on_art_pregnant"][ti] = np.count_nonzero(
                self.on_art & self.sim.people.pregnancy.pregnant
            )
        self.results["p_on_art"][ti] = sc.safedivide(
            self.results["n_on_art"][ti], self.results["n_infected"][ti]
        )

        # Subset by age group:
        infected_15_19 = self.infected[
            (self.sim.people.age >= 15) & (self.sim.people.age < 50)
        ]
        ...
        if "structuredsexual" in self.sim.networks.keys():
            ...
            if len(fsw_infected) > 0:
                self.results["prevalence_sw"][ti] = sum(fsw_infected) / len(
                    fsw_infected
                )
                self.results["new_infections_sw"][ti] = len(
                    (
                        (self.ti_infected == ti)
                        & self.sim.networks.structuredsexual.fsw
                    ).uids
                )
                self.results["new_infections_not_sw"][ti] = len(
                    (
                        (self.ti_infected == ti)
                        & ~self.sim.networks.structuredsexual.fsw
                    ).uids
            ...
            if len(client_infected) > 0:
                self.results["prevalence_client"][ti] = sum(client_infected) / len(
                    client_infected
                )
                self.results["new_infections_client"][ti] = len(
                    (
                        (self.ti_infected == ti)
                        & self.sim.networks.structuredsexual.client
                    ).uids
                )
                self.results["new_infections_not_client"][ti] = len(
                    (
                        (self.ti_infected == ti)
                        & ~self.sim.networks.structuredsexual.client
                    ).uids
    ...
    def start_art(self, uids):
        ...
        future_zero = uids[
            self.ti_zero[uids] > ti
        ]  # NB, if they are scheduled to die on this time step, they will
        ...
        if len(newly_treated) > 0:
            ...
            cd4_scale_factor = (
                (cd4_max - cd4_preart) / cd4_healthy * np.log(cd4_max / cd4_preart)
            )
            cd4_total_gain = cd4_preart * cd4_scale_factor
            self.cd4_potential[newly_treated] = (
                self.cd4_preart[newly_treated] + cd4_total_gain
            )
    ...
    def stop_art(self, uids=None):
        ...
        if uids is None:
            uids = self.on_art & (self.ti_stop_art <= ti)
        self.on_art[uids] = False
        ...
        # Set decline
        dur_mean = (
            np.log(self.cd4_preart[uids]) * self.cd4[uids] / self.cd4_potential[uids]
        )
