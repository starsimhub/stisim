...
class StructuredSexual(ss.SexualNetwork):
    ...
    def __init__(self, key_dict=None, condom_data=None, name=None, **kwargs):
        ...
        self.define_pars(
            ...
            p_pair_form=ss.bernoulli(
                p=0.5
            ),  # Probability of a (stable) pair forming between two matched people
            match_dist=ss.bernoulli(
                p=0
            ),  # Placeholder value replaced by risk-group stratified values below
            p_matched_stable=[
                0.9,
                0.5,
                0,
            ],  # Probability of a stable pair forming between matched people (otherwise casual)
            p_mismatched_casual=[
                0.5,
                0.5,
                0.5,
            ],  # Probability of a casual pair forming between mismatched people (otherwise instantanous)
        ...
        self.define_states(
            ...
            ss.FloatArr(
                "partners_12", default=0
            ),  # Number of partners over the past 12m
            ss.FloatArr(
                "lifetime_partners", default=0
            ),  # Lifetime total number of partners
    ...
    def process_condom_data(condom_data):
        if sc.isnumber(condom_data):
            ...
            for pcombo in df.partnership.unique():
                key = (
                    tuple(map(int, pcombo[1:-1].split(",")))
                    if pcombo != "(fsw,client)"
                    else ("fsw", "client")
                )
    ...
    def get_age_risk_pars(self, uids, par):
        ...
        for a_label, (age_lower, age_upper) in self.pars.f_age_group_bins.items():
            for rg in range(self.pars.n_risk_groups):
                in_risk_group = (
                    (self.sim.people.age[uids] >= age_lower)
                    & (self.sim.people.age[uids] < age_upper)
                    & (self.risk_group[uids] == rg)
                )
        ...
        if self.condom_data is not None:
            if isinstance(self.condom_data, dict):
                for rgtuple, valdict in self.condom_data.items():
                    self.condom_data[rgtuple]["simvals"] = sc.smoothinterp(
                        sim.t.yearvec, valdict["year"], valdict["val"]
                    )
        # self.init_results()
        return
    ...
    def match_pairs(self, ppl):
        ...
        f_looking = self.pars.p_pair_form.filter(
            f_eligible.uids
        )  # ss.uids of women looking for partners
        ...
        desired_ages = (
            ppl.age[f_looking] + age_gaps
        )  # Desired ages of the male partners
        m_ages = ppl.age[m_eligible]  # Ages of eligible males
        dist_mat = spsp.distance_matrix(
            m_ages[:, np.newaxis], desired_ages[:, np.newaxis]
        )
    ...
    def add_pairs(self, ti=None):
        ...
        acts = (self.pars.acts.rvs(p2)).astype(
            int
        )  # Number of acts per timestep - does not depend on commitment/risk group
        dur = np.full(
            len(p2), dtype=ss_float_, fill_value=dt
        )  # Default duration is dt, replaced for stable matches
        age_p1 = ppl.age[p1]
        age_p2 = ppl.age[p2]
        ...
        for rg in range(self.pars.n_risk_groups):
            p_match[matched_risk & (self.risk_group[p1] == rg)] = (
                self.pars.p_matched_stable[rg]
            )
            p_match[mismatched_risk & (self.risk_group[p2] == rg)] = (
                self.pars.p_mismatched_casual[rg]
            )
        ...
        # Checks
        if (self.sim.people.female[p1].any() or self.sim.people.male[p2].any()) and (
            self.name == "structuredsexual"
        ):
    ...
    def match_sex_workers(self, ppl):
        ...
        if len(m_looking) > len(active_fsw.uids):
            ...
            if len(fsw_repeats) < len(m_looking):
                fsw_repeats = np.repeat(
                    fsw_repeats, 10
                )  # 10x the number of clients each sex worker can have
    ...
    def end_pairs(self):
        ...
        self.edges.dur = (
            self.edges.dur - 1
        )  # Decrement the duration of each partnership, noting that dur is timesteps

        # Non-alive agents are removed
        alive_bools = (
            people.alive[ss.uids(self.edges.p1)] & people.alive[ss.uids(self.edges.p2)]
        )
        ...
        if self.pars.store_register:
            ...
            self.breakup_register = self.breakup_register[
                :11
            ]  # Forget partners from >12m ago
    ...
    def update_results(self):
        ti = self.ti
        self.results.share_active[ti] = len(self.active(self.sim.people).uids) / len(
            self.sim.people
        )
    ...
    def net_beta(self, disease_beta=None, uids=None, disease=None):
        ...
        p_trans_condom = (1 - disease_beta * (1 - eff_condom)) ** (
            self.edges.acts[uids] * p_condom
        )
        p_trans_no_condom = (1 - disease_beta) ** (
            self.edges.acts[uids] * (1 - p_condom)
        )
            ...
            if isinstance(self.condom_data, dict):
                for rgm in range(self.pars.n_risk_groups):
                    for rgf in range(self.pars.n_risk_groups):
                        risk_pairing = (self.risk_group[self.p1] == rgm) & (
                            self.risk_group[self.p2] == rgf
                        )
                        self.edges.condoms[risk_pairing] = self.condom_data[(rgm, rgf)][
                            "simvals"
                        ][self.ti]
                self.edges.condoms[self.edges.sw] = self.condom_data[("fsw", "client")][
                    "simvals"
                ][self.ti]
