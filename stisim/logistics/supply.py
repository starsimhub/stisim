...
class Supply:
    ...
    def __init__(self, quantity: int, product: Product):
        ...
        Args:
            quantity: (int) How much of the product is available in this supply. Must be a non-negative finite
                integer count, or np.inf to model an effectively unlimited/infinite supply. Passing np.inf means
                use() will never raise InsufficientSupplyException and the quantity remains np.inf, while usage cost
                still accrues normally. Arbitrary (non-integer) floats are not permitted.
        ...
        if not (isinstance(quantity, int) or quantity == np.inf):
            raise ValueError(f"Supply quantity must be an int (finite count) or np.inf (unlimited), "
                             f"got {quantity!r}.")
        if quantity < 0:
            raise ValueError(f"Supply quantity must be non-negative, got {quantity}.")
