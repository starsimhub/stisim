...
Usually a **target** where reliable estimates exist. Cohort-based incidence is preferable to biomarker-imputed incidence, which is itself a modelled quantity. Note carefully whether the source is a direct measurement or a Spectrum-style estimate.
...
## Sexual network and transmission behaviour
...
These are typically the **behavioural / network** category — poorly identifiable from direct evidence, influential on observable epidemiological patterns.
...
**Also check the background-mortality double-count.** If `ss.Deaths` is being fed all-cause data and `sti.HIV` is also killing agents from AIDS, mortality is double-counted. Use `stisim.data.dedup_deaths` before calibrating any mortality knob — otherwise the calibration will chase a data artefact. This is a **data preprocessing** step, not a calibration step.
...
## Behavioural parameters with country-specific evidence
