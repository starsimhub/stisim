...
The canonical failure pattern: a local edit to an editable dep's source works for months. A later version bump replaces the file. The bug returns. Debugging eventually traces back to the version bump — the fix had never been in the dep's git history to begin with, so nothing was "wiped", it just was never there to survive. The deeper cost, visible only in aggregate, is that every other user of the dep hit the same underlying bug for the months the fix sat private.
