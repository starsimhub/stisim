...
A minor rather than patch release: it requires `starsim>=3.6.0`, and the bugs found while migrating to it change simulated epidemiology and reported results enough that chlamydia, gonorrhea, trichomoniasis and syphilis calibrations all need redoing.
...
- `ss.Infection.infectious` is now an alias rather than a property; `Syphilis.infectious` and `HIV.symptomatic` still override it, so no change was needed. `Module.define_aliases()` also allows a *derived* state (a callable that gets its own automatic `n_<name>` result), which is what `SEIS.infected` now is.
- Starsim also fixed `prevalence` to use `people.n_alive` rather than `len(people)` as the denominator, which previously included agents who died on the current timestep while the numerator excluded them. STIsim overwrites the base `prevalence` result with its own age-restricted version, so this only affects diseases that don't (currently none).
