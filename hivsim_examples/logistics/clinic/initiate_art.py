...
class InitiateArt(SuppliedIntervention):
    def distribute_event(self, uptakers: ss.uids, product_name: str, product_quantity_per_agent: int):
        # apply the effect of "initiate art" to the uptakers, modifying their state
        ...
        # Both are derived from the 'Supplies' dict key of the event, e.g.: 'Supplies': {'dolutegravir': 1}
    ...
    def step(self):
        ...
        # From the 'Supplies' dict key of the event, e.g.: 'Supplies': {'dolutegravir': 1}
        product_name = 'dolutegravir'
        ...
        self.parent.initiate_art_selected = event_selected
        self.parent.initiate_art_results = event_results
        return
        # End template.
