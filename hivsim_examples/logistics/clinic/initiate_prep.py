...
class InitiatePrep(SuppliedIntervention):
    def distribute_event(self, uptakers: ss.uids, product_name: str, product_quantity_per_agent: int):
        # apply the effect of "initiate prep" to the uptakers, modifying their state
        ...
        # Both are derived from the 'Supplies' dict key of the event, e.g.: 'Supplies': {'oral': 1}
    ...
    def step(self):
        ...
        # From the 'Supplies' dict key of the event, e.g.: 'Supplies': {'oral': 1}
        product_name = 'oral'
        ...
        self.parent.initiate_prep_selected = event_selected
        self.parent.initiate_prep_results = event_results
        return
        # End template.
