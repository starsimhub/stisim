...
# product_name: 'hiv_test'
# product_quantity: 1
# event_name_downcased: queue_for_hiv_test


class QueueForHivTest(SuppliedIntervention):
    def distribute_event(self, uptakers: ss.uids, product_name: str, product_quantity_per_agent: int):
        # apply the effect of "queue for hiv test" to the uptakers, modifying their state
        ...
        # Both are derived from the 'Supplies' dict key of the event, e.g.: 'Supplies': {'hiv_test': 1}
    ...
    def step(self):
        ...
        # From the 'Supplies' dict key of the event, e.g.: 'Supplies': {'hiv_test': 1}
        product_name = 'hiv_test'
        product_quantity_per_agent = 1
        ...
        self.parent.queue_for_hiv_test_selected = event_selected
        self.parent.queue_for_hiv_test_results = event_results
        return
        # End template.
