...
default_folders = ['tutorials', 'examples'] # Folders with Jupyter notebooks
...
def update_version(pkg=sti):
    sc.heading('Updating docs version number...')
    filename = '_variables.yml'
    data = dict(version=sti.__version__, versiondate=sti.__versiondate__)
...
def customize_aliases(mod_name='stisim', json_path='objects.json'):
    """
...
def build_objects_inv(json_path='objects.json', inv_path='objects.inv'):
    ...
    inv = soi.Inventory()
    inv.project = data.get('project', 'stisim')
    inv.version = str(data.get('version', sti.__version__))
