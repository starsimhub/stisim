...
Here we provided specific keys to plot (`ng.new_infections` and `ng.prevalence`). You can also call `sim.plot('ng')` to see all the high-level gonorrhea results, or just `sim.plot()` to see everything. For more on working with results, see the [Results and plotting](tut_results.qmd) tutorial.
