...
Any module can define custom results: **analyzers**, **interventions**, **networks**, **connectors**, and **diseases** all use the same `define_results` pattern. The result then automatically appears in `sim.results`, `sim.to_df()`, and `sim.plot()` -- and can be used as a [calibration target](tut_calibration.ipynb).
...
3. After `sim.run()`, the result is available everywhere: `sim.results`, `sim.to_df()`, `sim.plot()`, and as a calibration target (see [Calibration tutorial](tut_calibration.ipynb)).
