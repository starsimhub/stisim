...
- For more elaborate downstream actions — for example, applying a specific
  ...
  :class:`~stisim.PartnerNotification` and override
  :meth:`~stisim.PartnerNotification.notify_attendees`. The default
  implementation schedules ``self.test`` for attendees; you can replace it
  with anything (set treatment eligibility, route by sex, send to a registry).
